...
function registerRingOfFireHandlers() {
  return {
    ...
    async onStart(players) {
      try {
        const logic = new RingOfFireLogic();
    ...
    async onPlayerAction(io, game, action, data, callback) {
      const logic = new RingOfFireLogic(game);
...
module.exports = registerRingOfFireHandlers();
